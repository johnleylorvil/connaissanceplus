...
const ROLE_LABEL: Record<AdminUserRole, string> = { student: 'Étudiant', moderator: 'Modérateur', admin: 'Administrateur' }
const emptyData: AdminUsersResponse = { items: [], pagination: { page: 1, pageSize: 25, total: 0, totalPages: 0 }, countsByRole: { student: 0, moderator: 0, admin: 0 }, countsByStatus: { active: 0, suspended: 0 } }
...
export default function AdminUsersView({ token, currentUserId, mode, onCreateModerator }: Props) {
  ...
  const cards = mode === 'team'
    ...
    : [{ label: 'Tous', value: data.countsByRole.student + data.countsByRole.moderator + data.countsByRole.admin }, { label: 'Étudiants', value: data.countsByRole.student }, { label: 'Modérateurs', value: data.countsByRole.moderator }, { label: 'Administrateurs', value: data.countsByRole.admin }, { label: 'Suspendus', value: data.countsByStatus.suspended }]
  ...
  return <div>
    ...
    <div className="card" style={{ marginBottom: 16 }}>
      <form onSubmit={submitSearch} style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
        ...
        <select className="field-input" style={{ flex: '1 1 160px' }} value={role} onChange={(e) => { setRole(e.target.value as AdminUserRole | ''); setPage(1) }}><option value="">Tous les rôles</option>{mode !== 'team' && <option value="student">Étudiants</option>}<option value="moderator">Modérateurs</option><option value="admin">Administrateurs</option></select>
