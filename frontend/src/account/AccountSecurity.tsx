...
export default function AccountSecurity({ token }: { token: string }) {
  ...
  return (
    <div className="account-page">
      ...
      <form className="card account-card account-form" onSubmit={submit}>
        <div><h2>Changer le mot de passe</h2><p>Votre mot de passe doit contenir au moins six caractères.</p></div>
        ...
        <label><span className="field-label">Nouveau mot de passe</span><input required minLength={6} type="password" autoComplete="new-password" className="field-input" value={form.newPassword} onChange={(event) => setForm({ ...form, newPassword: event.target.value })} /></label>
        <label><span className="field-label">Confirmer le nouveau mot de passe</span><input required minLength={6} type="password" autoComplete="new-password" className="field-input" value={form.confirmPassword} onChange={(event) => setForm({ ...form, confirmPassword: event.target.value })} /></label>
